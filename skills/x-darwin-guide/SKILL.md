...
1. **Identify and load relevant documentation**
   ...
   - **Always read:** [architecture.md](knowledge/architecture.md) - Core patterns (POST-only APIs, NestJS structure, monorepo layout)
   - **Always read:** [idiosyncrasies.md](knowledge/idiosyncrasies.md) - Monorepo gotchas and workarounds
   - Creating new service/module/entity? → READ [creating-new-modules.md](knowledge/creating-new-modules.md)
   - Console-API (BFF/GraphQL)? → READ [console-api-patterns.md](knowledge/console-api-patterns.md)
   - Console-Web (Frontend)? → READ [console-web-patterns.md](knowledge/console-web-patterns.md)
   - Client config changes? → READ [client-config.md](knowledge/client-config.md)
...
4. **Route to specialized knowledge when needed**
   - Client config versioning workflow → Use [client-config-versioning.md](knowledge/client-config-versioning.md) step-by-step
   - Browser testing → Invoke `/browser-testing` skill
   - Application service validation/requirements → Use [application-service-patterns.md](knowledge/application-service-patterns.md)
